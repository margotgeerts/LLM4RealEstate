...
def gather_answers(idx, prompt, answers, model='neuralmagic/Meta-Llama-3.1-70B-Instruct-FP8', n_examples=0, context=False, example_selection='geo', n_gpu=1, output_file=None):
    ...
    if len(idx) == 1:
        messages = [[{"role": "system", "content": prompt.system_prompt},
                ...
                {"role": "assistant", "content": f"{str(answers.loc[idx, 'min'])} - {str(answers.loc[idx, 'max'])}"},
                {"role": "user",
        ...
        messages = [[{"role": "system", "content": prompt.system_prompt},
                ...
                {"role": "user", "content": "Please provide a price interval with 90% coverage around your estimated price for this property in the format 'min_price - max_price'."},
                {"role": "assistant", "content": f"{answers.loc[i,'min']} - {answers.loc[i,'max']}"},
                {"role": "user",
                        "content": "Please provide the top 5 features that you deemed most important for your previous predictions. Answer with a comma-separated list of features, using the feature names: {features}.".format(features=', '.join(['coordinates', 'address']+prompt.features['hedonic']+['transaction_date']))}
    ...
    for i in range(0, len(input_list_tok), chunk_size):
        ...
        answers.loc[idx[start_id:end_id], 'features'] = output_list
