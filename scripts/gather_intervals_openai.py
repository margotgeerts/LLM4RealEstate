...
def gather_answers(batch_idx, prompt, answers, model='gpt-4o-mini', n_examples=0, context=False, example_selection='geo', n_chunks=2):
    ...
    print(answers.shape)
    if 'min_price' in answers.columns:
    ...
    for idx in batch_idx:
        ...
        batch_requests.append(
            {"custom_id": f"request2-{idx}", "method": "POST", "url": "/v1/chat/completions", 
    ...
    if n_chunks > 1:
        ...
        for i in range(n_chunks):
            ...
            with open(f'tmp/requests2{i+1}.jsonl', 'w') as f:
            ...
            batch_input_file = client.files.create(
                file=open(f"tmp/requests2{i+1}.jsonl", "rb"),
                purpose="batch"
                )
            ...
            intervals = [get_interval_from_response(get_content_from_response(r, f"request2-{idx}")) for idx in chunk_idx]
            mins = [interval[0] for interval in intervals]
            maxs = [interval[1] for interval in intervals]
            answers.loc[chunk_idx, 'min_price'] = mins
            answers.loc[chunk_idx, 'max_price'] = maxs
        ...
        # Save the requests to a file
        with open('tmp/requests2.jsonl', 'w') as f:
            # save each request as a separate line
        ...
        batch_input_file2 = client.files.create(
            file=open("tmp/requests2.jsonl", "rb"),
            purpose="batch"
            )
...
if __name__ == '__main__':
    ...
    batch_idx = [idx for idx in indices if (idx not in results.index) or (results.loc[idx].isna().sum()>=2) or ('min_price' not in results.columns)]
