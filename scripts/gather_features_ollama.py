...
def gather_answers(idx, prompt, results, model='llama3.2', n_examples=0, context=False, example_selection='geo'):
    ...
    min, max = results.loc[idx, 'min_price'], results.loc[idx, 'max_price']

    messages.append({"role": "assistant", "content": f"{min} - {max}"})
    messages.append({"role": "user", 
    ...
    features = get_features_from_response(response['message']['content'])
...
if __name__ == '__main__':
    ...
    # complete the following indices: either all if 'features' is not in the columns,
    # or the last ones that are not completed
    indices_to_complete = []
    if 'features' not in results.columns:
        indices_to_complete = indices
    else:
        # find the last index that is completed
        last_idx = results.index[results['features'].notna()].max()
        # complete the rest (indices that come after last_idx in the indices list)
    ...
    for idx in indices_to_complete:
        features = gather_answers(idx, prompt, results, args.model, args.examples, args.context, args.example_selection)
        results.loc[idx, 'features'] = ','.join(features)
